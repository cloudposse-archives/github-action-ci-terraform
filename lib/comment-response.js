class CommentResponse {
    ...
    respond() {
        ...
        rt = inputs.recognized_tests
        ut = inputs.unrecognized_tests
        rr = inputs.recognized_runs
        ur = inputs.unrecognized_runs
        ...
        const { data: comment } = githubClient.issues.getComment({
          owner: owner,
          repo: repo,
          comment_id: inputs.comment_id,
        });
...
module.exports = {
    CommentResponder: function (actionContext) {
        return new CommentResponder(actionContext)
    }
}
